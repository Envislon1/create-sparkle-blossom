...
const EnergyBarChart = () => {
  ...
  const chartData = deviceAssignments.map((device, deviceIndex) => {
    ...
    // Calculate total energy for this device across all channels (convert Wh to kWh)
    const deviceReadings = energyReadings.filter(r => r.device_id === device.device_id);
    const totalDeviceEnergy = deviceReadings.reduce((sum, reading) => sum + ((reading.energy_wh || 0) / 1000), 0);
    ...
    return deviceChannelsList.map((channel, channelIndex) => {
      ...
      // Convert Wh to kWh for channel energy
      const channelEnergy = channelReadings.reduce((sum, reading) => sum + ((reading.energy_wh || 0) / 1000), 0);
      ...
      return {
        ...
        value: Number(channelEnergy.toFixed(2)), // Already in kWh
